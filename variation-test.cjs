...
assert.equal(BANK.length,100);assert.equal(FINALS.length,80);assert.equal(new Set(ALL_CATEGORIES).size,100);
const all=BANK.flatMap(c=>c.levels.flat());assert.equal(all.length,2500);assert.equal(new Set(all.map(q=>q.id)).size,2500);assert.equal(new Set(all.map(q=>q.key)).size,2500);
...
assert.equal(new Set([...all,...FINALS].map(q=>q.clue)).size,2580);
...
assert.equal(covered.size,2500);assert.equal(seenFinals.size,80);`);
...
const old=files.map(f=>cp.execFileSync('git',['show','72dab6fc5fb25640839b4c3d20cec3a5f395c473:dist/'+f],{cwd:__dirname,encoding:'utf8'})).join('\n');
for(const phase of ['board','active','final']){
...
const result={games:1000,ordinaryPlayed:90000,categoryChanges:6000,ordinaryAlternativesObserved:2500,finalAlternativesObserved:80,reloadCases:6,legacySaveCompatibility:true,excessiveCategorySwaps:true};
